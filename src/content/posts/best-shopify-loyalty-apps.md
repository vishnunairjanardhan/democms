---
pubDate: 2025-19-12
author: Piyush
authorImg: "Piyush.webp"
title: "12 Best Shopify Loyalty Apps for 2026"
description: Looking to boost customer retention in 2026? Check out the 12 best Shopify loyalty apps for seamless rewards, gamification, and engagement strategies.
heading: "12 Best Shopify Loyalty Apps for 2026"
image:
  url: "/assets/blog/12_best_shopify_loyalty_apps_for_2026.webp"
  alt: "featured image for blog best shopify shopify loyalty apps"
tags: ["Shopify"]
url: "https://www.99minds.io/blog/best-shopify-loyalty-apps"
keywords: ["Shopify loyalty apps"]

--- 

<div class="lg:flex lg:gap-6">
  <!-- Table of content section -->
  <div class="lg:w-[250px] lg:sticky top-40 self-start shrink-0 p-[0.060rem] lg:mt-2 h-full text-black pb-4 overflow-y-auto max-h-[80vh]">
    <p class="text-black font-semibold text-nowrap pl-0">Table of Contents:</p>
<ul class="pl-0 text-base list-none">
  <li class="hover:bg-gray-100 lg:mt-4">   
    <a role="link" href="#1" rel="noreferrer">12 Best Loyalty Apps for Shopify in 2026</a>
  </li>           
 <li class="hover:bg-gray-100 lg:mt-4">  
    <a role="link" href="#2" ref="noreferrer">Conclusion</a>
  </li>  
  <li class="hover:bg-gray-100 lg:mt-4"> 
    <a role="link" href="#3" ref="noreferrer">Frequently Asked Questions (FAQs) on Shopify Loyalty Apps</a>
  </li>  
</ul>
</div>

<div class="w-full">
<img rel="preload" src="/assets/blog/12_best_shopify_loyalty_apps_for_2026.webp" alt="blog banner image for best shopify loyalty apps">

If you want more <a href="https://www.99minds.io/blog/repeat-purchases" role="link" target=_blanck>repeat purchases</a>", higher customer lifetime value, and a dependable way to retain shoppers without relying heavily on paid ads, a strong <a href="https://www.99minds.io/blog/loyalty-program" role="link" target="_blanck">loyalty program</a> is one of the smartest moves a Shopify brand can make.

As competition grows across ecommerce and physical retail, focusing only on customer acquisition is no longer sustainable. Brands that win long term are the ones that invest in retention and build meaningful relationships with their customers. For Shopify merchants selling through both online stores and brick-and-mortar locations, loyalty has become a core growth strategy rather than a nice-to-have.

That’s where Shopify loyalty apps come in. From points and referrals to omnichannel rewards, store credit, and VIP tiers, the right loyalty app can turn one-time shoppers into loyal, repeat customers. 


<h2 id="1">12 Best Loyalty Apps for Shopify in 2026</h2>

### 1. 99minds

<div class="m-auto w-full lg:px-33">
    <img loading="lazy" role="img" src="/assets/blog/99minds-shopify-loyalty-app.webp" alt="99minds loyalty app on shopify">
</div>

99minds is the best Shopify <a href="https://www.99minds.io/loyalty" role="link" target="_blanck">loyalty app</a> for omnichannel DTC brands because of its all-in-one retention stack, deep omnichannel capabilities, and flexibility across eCommerce and physical stores.

Unlike basic point-based tools, 99minds is built as a full-scale customer retention platform that unifies loyalty, gift cards, memberships, referrals, and store credits into a single system. It ensures customers earn, redeem, and engage seamlessly whether they shop online, in-store, or through mobile wallets, making loyalty consistent and measurable across every touchpoint.

Designed for modern DTC brands operating on Shopify, it helps brands move beyond discounts and build long-term customer relationships that directly improve repeat purchase rate and lifetime value.

**Key Features:**

- **Custom Loyalty Experience:** Fully customizable loyalty widget and branded loyalty page ensure rewards feel native to your store, not bolted on.

- **Digital & Physical Gift Cards:** Offer both online and in-store gift cards to drive gifting, repeat purchases, and omnichannel revenue.

- **Store Credits for Returns:** Convert refunds into store credits to retain revenue and bring customers back instead of losing them.

- **Tiered Rewards & Memberships:** Paid and free membership tiers unlock exclusive rewards, perks, and benefits that encourage higher spend.

- **Referral Program:** Customers share referral links and earn rewards, turning loyal buyers into a scalable acquisition channel.

- **Unified Digital Wallet:** Rewards, gift cards, and store credits live in one wallet for easy access and higher redemption rates.

- **Automated Loyalty Engine:** A powerful automation engine rewards actions, purchases, and milestones without manual effort.

**Pros and Cons:**

99minds’ biggest advantage is its true omnichannel loyalty system that unifies rewards, gift cards, store credits, memberships, and referrals across online and physical stores. It replaces multiple retention tools with one platform. The downside is that very small, online-only stores may find its advanced features unnecessary without proper setup and strategy.

<a href="https://www.99minds.io/pricing" role="link" target="_blanck">Pricing</a>:

- Free to Install 

<div class="flex justify-center w-full mt-0 mb-4">
  <div class="w-full">
    <div class="bg-[#AA8FFF40] rounded-2xl px-12 flex flex-col justify-between items-center w-full">
      <div class="text-center">
        <h2 class="text-2xl font-semibold text-gray-900">
          Turn One-Time Buyers into Loyal Customers
        </h2>
        <p class="text-gray-600 mt-2">
          With 99minds, create unified loyalty programs that work seamlessly across online and physical stores
        </p>
      </div>
      <div class="flex space-x-4 my-6 lg:mt-0">
        <button class="px-4 py-2 font-semibold border border-gray-300 rounded-lg text-gray-800 bg-white hover:bg-gray-100">
          <a href="https://calendly.com/99minds/30min" class="no-underline">
            Book a Demo
          </a>
        </button>
        <a href="/Get-Started" class="px-4 py-2 no-underline font-semibold bg-[#7F56D9] text-white rounded-lg hover:bg-purple-700">
          Get started
        </a>
      </div>
    </div>
  </div>
</div>

### 2. Smile.io

<div class="m-auto w-full lg:px-33">
    <img loading="lazy" role="img" src="/assets/blog/smile-shopify-loyalty-app.webp" alt="Smile loyalty app on shopify">
</div>

Smile.io is one of the most widely used Shopify loyalty apps, built for brands that want a customized loyalty program with points, referrals, and VIP programs without heavy setup. It’s best suited for small to mid-sized DTC brands that are starting with loyalty and want a proven, easy-to-manage solution.

**Key Features:**

- **Points-Based Rewards:** Customers earn points for purchases and actions, encouraging repeat buying without heavy discounts.

- **Referral Growth Engine:** Double-sided referral rewards turn loyal customers into an acquisition channel.

- **VIP Tier System:** Tiered perks reward high-value customers and push higher lifetime value.

- **Branded Loyalty Touchpoints:** Custom widgets and automated emails keep loyalty visible across the customer journey.

- **Shopify-Native Setup:** Built specifically for Shopify to ensure smooth performance and easy installation.

**Pros and Cons:**

Smile.io stands out for its ease of use, fast setup, and reliable points, referrals, and VIP programs that work well for Shopify beginners. It’s stable and widely trusted. However, it lacks advanced automation, deep omnichannel workflows, and store credit features, making it less suitable for complex or retail-heavy DTC brands.

**Pricing Plans:**

- Free to Install
- **Starter plan:** $49/month
- **Growth plan:** $199/month
- **Plus Plan:** $999/month

### 3. Rise.ai: Gift Cards & Loyalty

<div class="m-auto w-full lg:px-33">
    <img loading="lazy" role="img" src="/assets/blog/rise-shopify-loyalty-app.webp" alt="Rise loyalty app on shopify">
</div>

Rise.ai is a Shopify store credit platform built to increase sales and repeat purchases through gift cards, cashback, rewards, and refunds. It turns store credit into a powerful retention and acquisition engine by giving customers real purchasing power, helping brands boost conversions, increase AOV, and recover revenue that would otherwise be lost to refunds and churn.

**Key Features**

- **Unified Store Credit Wallet:** Combine gift cards, loyalty rewards, cashback, refunds, and compensation into one branded customer wallet.

- **Cashback Campaigns:** Run cashback offers that incentivize higher order values and encourage repeat purchases.

- **Store Credit Rewards:** Reward customers with credit for actions like signups, lifetime spend, referrals, and promotions.

- **Digital & Physical Gift Cards:** Sell customizable gift cards with reminder flows to drive redemption and repeat visits.

- **Refunds & Compensation Credits:** Issue store credit for refunds or service recovery to retain revenue and reduce churn.

**Pros and Cons:**

Rise.ai excels at turning refunds, cashback, and gift cards into retained revenue through a unified store credit wallet. It’s especially effective for reducing churn and increasing repeat purchases. The limitation is that it’s not a full loyalty platform, offering fewer options for tiered rewards, gamification, or advanced engagement programs.

**Pricing Plans:**

- **Starter plan:** US $19.99/month
- **Small‑Business plan:** US $59.99/month
- **Pro plan:** US $199.99/month

### 4. Yotpo: Loyalty & Rewards

<div class="m-auto w-full lg:px-33">
    <img loading="lazy" role="img" src="/assets/blog/yotpo-shopify-loyalty-app.webp" alt="Yotpo loyalty app on shopify">
</div>

Yotpo Loyalty & Rewards is a flexible Shopify loyalty solution built for growing DTC brands that want data-driven retention. It helps merchants increase repeat purchases and engagement through customizable rewards, referrals, and VIP programs powered by advanced analytics.

**Key Features**

- **Points, Rewards & VIP Tiers:** Create loyalty programs that reward purchases, spending milestones, and customer engagement with flexible points and tier-based benefits.

- **Referral Programs:** Incentivize existing customers to acquire new ones through referral rewards that drive high-intent traffic.

- **No-Code Campaign Management:** Launch and modify loyalty campaigns quickly using pre-built templates without any technical setup.

- **Action-Based Rewards:** Reward customers for meaningful actions like referrals, social engagement, and repeat visits, not just transactions.

- **Customer Segmentation:** Target loyalty members based on points balance, referral activity, purchase behavior, and lifecycle stage.

- **Advanced Analytics & Dashboards:** Track revenue impact, engagement trends, and loyalty performance to continuously optimize your rewards strategy.

**Pros and Cons:**

Yotpo’s strength lies in its powerful customization, analytics, and tight integration with Yotpo’s reviews and SMS tools, making it ideal for data-driven brands. However, it comes with premium pricing and complexity, which may be excessive for smaller teams or brands not already invested in the Yotpo ecosystem.

**Pricing Plans:**

- Free to Install
- **Pro Plan:** $199/month
- **Premium Plan:** $799/month

### 5. Joy Rewards & Loyalty Program

<div class="m-auto w-full lg:px-33">
    <img loading="lazy" role="img" src="/assets/blog/joy-rewards-shopify-loyalty-apps.webp" alt="Joy loyalty app on shopify">
</div>

Joy Rewards & Loyalty Program is a flexible Shopify loyalty app built for growing DTC brands that want control without complexity. It helps merchants design customizable loyalty programs with points, VIP tiers, referrals, and fully branded experiences, making it easier to retain customers and increase repeat purchases.

**Key Features**

- **Unlimited Points Rewards:** Reward customers for purchases, referrals, and engagement actions using flexible point rules.

- **On-Brand Loyalty Experience:** Customize loyalty widgets, popups, and pages to match your store’s design across multiple touchpoints.

- **VIP Tiers & Memberships:** Create tiered loyalty programs with exclusive perks to encourage higher spending and long-term loyalty.


- **Extensive Integrations:** Works with Shopify POS, checkout redemptions, and essential integrations to support omnichannel loyalty.

- **Dedicated Support:** Access 24/7 live chat support to resolve issues and optimize your loyalty setup quickly.

**Pros and Cons:**

Joy Rewards offers strong customization, flexible points rules, and solid POS support at an accessible price point. It’s a good fit for growing brands that want control without enterprise cost. The downside is limited advanced analytics and automation compared to higher-end platforms, which may restrict optimization at scale.

**Pricing Plans:**

- **Starter Plan:** Free
- **Essential Plan:** $24.99/month
- **Advanced Plan:** $99/month
- **Ultimate Plan:** $499/month 

### 6. Gameball: Loyalty, Points, VIP

<div class="m-auto w-full lg:px-33">
    <img loading="lazy" role="img" src="/assets/blog/gameball-shopify-loyalty-app.webp" alt="Gameball loyalty app on shopify">
</div>

Gameball is a gamified Shopify loyalty platform designed to increase engagement, not just repeat purchases. It helps DTC brands move beyond basic points by using challenges, badges, VIP tiers, and interactive mechanics that motivate customers to participate, return frequently, and build long-term brand loyalty.

**Key Features**

- **Points-Based Loyalty Program:** Reward customers with points, coupons, store credits, freebies, and VIP status for purchases and actions.

- **Action-Driven Rewards:** Incentivize signups, reviews, social follows, referrals, and newsletter subscriptions to boost engagement beyond orders.

- **Referral Program:** Encourage customers to invite friends with referral rewards that drive high-intent traffic.

- **Gamification Engine:** Engage shoppers with challenges, badges, leaderboards, streaks, and interactive elements like spin-the-wheel.

- **Fully On-Brand Experience:** Customize colors, text, fonts, and widgets to match your brand identity across touchpoints.

**Pros and Cons:**

Gameball’s biggest advantage is its gamification engine, using challenges, badges, and interactive elements to drive engagement beyond discounts. It’s great for engagement-focused brands. The trade-off is increased setup complexity and a learning curve, which may not suit brands seeking simple, low-maintenance loyalty programs.

**Pricing Plans:**

- Free Plan Available
- **Starter Plan:** $34/month
- **Pro Plan:** $159/month

### 7. LoyaltyLion Rewards & Referral

<div class="m-auto w-full lg:px-33">
    <img loading="lazy" role="img" src="/assets/blog/loyaltylion-shopify-app.webp" alt="Loyalty Lion loyalty app on shopify">
</div>

LoyaltyLion is a robust Shopify loyalty platform built for growing and established DTC brands that want full control over retention. It helps merchants increase repeat purchases, reduce acquisition costs, and improve customer experience through points, rewards, referrals, and deep behavioral insights.

**Key Features**

- **Custom Loyalty Page Integration:** Embed a fully branded loyalty page directly into your storefront and customer journey.

- **Points-Based Enrollment:** Use loyalty points to drive sign-ups, secure second purchases, and encourage repeat buying.

- **Flexible Rewards Engine:** Create rewards that motivate repeat purchases and higher program participation.

- **Engagement Notifications:** Drive ongoing interaction through loyalty emails and notifications tied to customer behavior.

- **Advanced Segmentation & Insights:** Identify churn risks and optimize retention using detailed data on returning customer behavior.

**Pros and Cons:**

LoyaltyLion excels in advanced rule-based rewards, deep analytics, and precise control over loyalty mechanics, making it ideal for mature DTC brands. However, its higher pricing and steeper learning curve can be a barrier for smaller teams or brands without dedicated resources to manage optimization.

**Pricing Plans:**

- Free to Install
- **Classic Plan:** $199/month

### 8. Rivo Loyalty Rewards Referrals

<div class="m-auto w-full lg:px-33">
    <img loading="lazy" role="img" src="/assets/blog/rivo-shopify-loyalty-app.webp" alt="Rivo loyalty app on shopify">
</div>

Rivo is a modern Shopify loyalty platform built for fast-growing DTC brands that want measurable retention without complexity. It focuses on loyalty, rewards, and referrals that actually move repeat purchase metrics, with frequent product updates, seamless integrations, and a customer success team dedicated to improving real retention outcomes.

**Key Features**

- **Fully Customizable Loyalty Platform:** Build loyalty and referral programs that are flexible, on-brand, and designed to improve retention metrics.

- **Built for Growing DTC Brands:** Optimized for Shopify stores scaling their loyalty and rewards strategy.

- **24/7 Live Chat Support:** Access a responsive customer success team focused on helping you drive real results.

- **Instant Integrations:** Connect easily with Klaviyo, Gorgias, Postscript, and other essential Shopify tools.

- **Developer Toolkit:** Unlock advanced customization and extended functionality using Rivo’s developer tools.

**Pros and Cons:**

Rivo stands out for its modern UI, frequent product updates, and strong focus on improving real retention metrics with excellent customer support. It integrates easily with popular Shopify tools. The limitation is that it offers fewer omnichannel and store-credit features compared to more comprehensive loyalty platforms.

**Pricing Plans:**

- Free Plan Available
- **Scale Plan:** $49/month
- **Plus Plan:** $499/month

### 9. BON Loyalty Rewards Referrals

<div class="m-auto w-full lg:px-33">
    <img loading="lazy" role="img" src="/assets/blog/bon-shopify-loyalty-app.webp" alt="Bon loyalty app on shopify">
</div>

BON Loyalty is a flexible Shopify loyalty platform built to help brands increase retention and repeat purchases with minimal setup. It enables merchants to launch points, VIP tiers, and referral programs quickly while delivering a branded loyalty experience across online and in-store channels, supported by ongoing customization and support.

**Key Features**

- **Effortless Setup:** Launch a complete loyalty points program in minutes without technical complexity.

- **VIP Tiers for Higher LTV:** Reward top customers with accelerated points, exclusive perks, and tier-based benefits.

- **On-Brand Loyalty Pages:** Integrate loyalty seamlessly across homepage, product pages, and checkout.

- **Omnichannel Rewards:** Allow customers to earn and redeem rewards both online and through Shopify POS.

- **B2B Loyalty Support:** Drive wholesale sales with exclusive tiered rewards designed for B2B customers.

**Pros and Cons:**

BON Loyalty’s key strength is its fast setup, omnichannel POS support, and flexibility for both B2C and B2B loyalty programs. It’s easy to launch and manage. However, it lacks advanced automation, deep analytics, and sophisticated engagement features required by enterprise-scale or highly customized loyalty strategies.

**Pricing Plans:**

- Free Plan Available
- **Basic Plan:** $25/month
- **Growth Plan:** $99/month
- **Professional Plan:** $349/month

### 10. TapMango  

<div class="m-auto w-full lg:px-33">
    <img loading="lazy" role="img" src="/assets/blog/tapmango-shopify-loyalty-app.webp" alt="Tapmango loyalty app on shopify">
</div>

TapMango is a Shopify-connected loyalty platform designed for brands with strong physical retail presence. It helps merchants create customizable loyalty programs that reward customers consistently across in-store and online channels, making it easier to increase repeat visits, manage rewards, and grow sales through a unified POS-friendly experience.

**Key Features**

- **Automatic Points Earning:** Customers earn loyalty points automatically for purchases made both in-store and online.

- **In-Store & Online Discounts:** Offer customized discounts that work seamlessly across physical locations and eCommerce.

- **Customer Rewards Widget:** A dedicated rewards widget lets customers view points, redeem rewards, and manage their loyalty account easily.

**Pros and Cons:**

TapMango is highly effective for physical-store-first brands, offering strong POS-based loyalty, in-store rewards, and SMS engagement. It works well for retail operations. The downside is a dated interface and limited eCommerce-focused features, making it less appealing for digitally native or online-first DTC brands.

**Pricing Plans:**

- Free to Install

### 11. Kangaroo: Loyalty and Rewards

<div class="m-auto w-full lg:px-33">
    <img loading="lazy" role="img" src="/assets/blog/kangaroo-shopify-loyalty-app.webp" alt="Kangaroo loyalty app on shopify">
</div>
 
Kangaroo is an omnichannel loyalty platform built for DTC and retail brands that want to increase sales, drive repeat visits, and own customer relationships. It enables brands to run personalized, data-driven loyalty programs across online stores and physical locations using rewards, referrals, and targeted engagement.

**Key Features**

- **Fully Branded Loyalty Experience:** Customize loyalty programs with your brand’s colors, logo, visuals, and messaging for a consistent experience.

- **Points, Offers & Redemptions:** Encourage repeat purchases with points per dollar spent, targeted offers, and flexible reward redemption.

- **Targeted Customer Engagement:** Reach customers with personalized offers through SMS, email, and push notifications.

- **Referral Engine:** Turn loyal customers into brand advocates by rewarding referrals that drive new traffic and sales.

**Pros and Cons:

Kangaroo shines in multi-location and omnichannel retail environments, offering centralized loyalty management, segmentation, and branded rewards. It’s ideal for complex retail setups. The drawback is higher complexity and pricing, which may feel excessive for small brands or single-location Shopify merchants.

**Pricing Plans:**

- Ultimate Plan: $199/month

### 12. Glow: Loyalty Points & Rewards

<div class="m-auto w-full lg:px-33">
    <img loading="lazy" role="img" src="/assets/blog/glow-shopify-loyalty-app.webp" alt="Glow loyalty app on shopify">
</div>

Glow is a Shopify-native loyalty app designed to help merchants turn first-time buyers into repeat customers. It offers a simple yet effective points and rewards system with referrals, seamless Shopify integration, and guided setup, making it easy for brands to launch loyalty programs without complexity.

**Key Features**

- **Points-for-Actions Rewards:** Increase customer lifetime value by rewarding purchases, referrals, and engagement actions with points.

- **Reward Redemptions:** Drive repeat sales by allowing customers to redeem points for discounts and exclusive offers.

- **Referral Incentives:** Encourage customer-led growth by rewarding both referrers and their friends.

- **Shopify POS Sync:** Enable customers to earn and redeem rewards across online stores and physical locations.

- **Loyalty Analytics:** Track engagement, redemption trends, and loyalty performance with actionable dashboards.

**Pros and Cons:**

Glow’s advantage is its simplicity and beginner-friendly setup, making it easy for small Shopify stores to test loyalty quickly. It integrates cleanly with Shopify and POS. However, it lacks advanced features, automation, and scalability, which limits its usefulness as brands grow and loyalty needs become more complex.

**Pricing Plans:**

- Free Plan Available
- **Sprout Plan:** $9/month
- **Startup Plan:** $29/month
- **Traction Plan:** $59/month

<h2 class="m-0 mb-4" id="2">Conclusion</h2>

We hope this list helped you see how the right Shopify loyalty app can directly impact repeat purchases, customer lifetime value, and long-term retention for DTC brands. Loyalty programs are no longer just about points. They are a proven way to reduce churn, strengthen brand relationships, and grow sustainably in a competitive market.

With a solution like <a href="https://www.99minds.io/" role="link" target="_blanck">99minds</a>, you can launch a complete loyalty engine that works across both your online store and physical locations. From tiered rewards and memberships to store credits, referrals, and a unified digital wallet, it gives you everything you need to turn one-time buyers into loyal, repeat customers.

<h2 class="m-0 mb-4" id="3">Frequently Asked Questions (FAQs) on Shopify Loyalty Apps</h2>

With these 12 Shopify loyalty apps, you can create a rewards program that fits your business’s unique needs. From simple point systems to complex tiered memberships, these apps provide everything you need to engage and retain your customers.

No matter the size of your business or your industry, the right loyalty app can make a significant difference in your customer retention and bottom line. Explore these options, try out their features, and see which one works best for your Shopify store. Here’s to a successful 2025 filled with loyal customers and growing revenue!

Ready to take your loyalty game to the next level? Start with <a href="https://www.99minds.io/" role="link" target="_blanck">99minds</a> today and experience the power of personalized customer retention.

<h2 id="Frequently-Asked-Questions-(FAQs)-on-Shopify-Loyalty-Apps">Frequently Asked Questions (FAQs) on Shopify Loyalty Apps</h2>

<div class="border border-vulcan-800 rounded-xl mt-6 px-8 pb-6"><h3>What is a Shopify loyalty app?</h3>A Shopify loyalty app is a tool integrated into your Shopify store to create, manage, and optimize loyalty programs, helping increase customer retention and repeat purchases.</div>

<div class="border border-vulcan-800 rounded-xl mt-6 px-8 pb-6"><h3>How do loyalty programs benefit my Shopify store?</h3>They improve customer retention, boost average order value, and encourage repeat purchases, ultimately increasing your store’s revenue.</div>

<div class="border border-vulcan-800 rounded-xl mt-6 px-8 pb-6"><h3>Can I customize the loyalty program for my brand?</h3>Yes, most Shopify loyalty apps allow extensive customization of rewards, points, branding, and more to align with your store’s identity.</div>


</div>
  <!-- CTA section  -->
 <div class="lg:w-[270px] lg:sticky top-40 self-start shrink-0 p-[0.060rem] lg:mt-5 h-full text-black pb-30 md:mx-0">
  <div class="rounded-lg mt-4 shadow-md bg-[linear-gradient(0deg,rgba(255,255,255,0.3),rgba(255,255,255,0.3)),linear-gradient(120.74deg,#FFB4B7_-1.19%,#DEB5E7_29.65%,#84B5FF_62.61%,#C68CFF_93.58%)] overflow-hidden flex flex-col">
    <div class="p-4">
      <h2 class="text-base !font-bold">
        Boost Customer Engagement with 99minds. The Ultimate Gift Card & Loyalty Solution!
      </h2>
      <a href="https://giftcard.99minds.io/access/register" 
         class="inline-block bg-black !text-white font-medium text-lg px-4 py-2 rounded-lg mt-4 hover:bg-gray-800 transition duration-300 no-underline"
         target="_blank">
        Signup for free
      </a>
    </div>
    <img src="/assets/blog/Dashboard.png"
         class="w-full h-auto ml-4 mb-0 pb-0">
  </div>
</div>



</div>